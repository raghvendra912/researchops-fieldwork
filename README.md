...
- `GET/PUT /api/projects/:projectId/ownership`
...
`GET /api/projects` supports `q`, `projectId`, `client`, `manager`, `salesPerson`, `status`, `type`, `from`, `to`, `page`, `pageSize`, `sortBy`, `sortDirection`, and `scope`. Project Center and Overview request `scope=all`; RLS still limits ANALYST/MEMBER to explicitly granted projects. Responses include total results, total pages, filter options, and portfolio status counts.

Client records are administered by OWNER/ADMIN roles and contain contact and outcome configuration. Supplier records contain contacts, STATIC/DYNAMIC redirect mode, outcome destinations, and two supplier-level base routes. The directory `/test` route sends synthetic identifiers to a configured supplier return destination as a configuration check; `/live` needs a project code and unique respondent ID. Project details generate the ready-to-send project-specific Test and Live launch links. The four separate client outcome links are for Complete, Terminate, Quota Full, and Security Terminate returns from the survey platform. Live launch links enforce project/supplier state, atomically reserve project/supplier and matching interlocked quota capacity, apply eligibility and fraud controls, record events, inject masked client callbacks, and return terminal outcomes to the supplier. Project-specific Test launches are counted separately and never reserve production quota. STATIC/DYNAMIC is currently stored as supplier metadata; vendor-specific redirect behavior and parameter contracts still require certification.

Respondent routing preserves two attempt identifiers: the supplier supplies its own unique reference in the project launch link, and ResearchOps creates a stable session UUID for that launch. The client survey receives the ResearchOps UUID as `{{respondent_id}}`/`rid`; its outcome return must send that UUID back. ResearchOps resolves the UUID to the stored supplier reference and returns the supplier reference, not the ResearchOps UUID, in the supplier destination. Client outcomes from surveys launched before this separation can still resolve the legacy supplier reference; ambiguous legacy references are rejected rather than attributed to the wrong supplier. The Respondents explorer and CSV show both IDs for reconciliation and accept searches by either ID. Migration `039` makes session and quota-reservation uniqueness assignment-scoped, so different suppliers may use the same external value on one project without sharing an internal session. Until `039` is applied, the router rejects a known cross-supplier collision. Apply `039` during a controlled database window and verify concurrent launches, quota accounting, terminal outcomes, and historical redirects before enabling external traffic with overlapping supplier values.
