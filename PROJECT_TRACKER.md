...
| Date | Check | Result |
|---|---|---|
| 2026-09-18 | `scripts/check-route.mjs` hardening | LOCAL PASS - replaced positional `argv[2]` parsing with full argument parsing (`--diagnose`/`--json` order-independent, `--timeout=<ms>` with 120s cap, `--help`/`-h`, `--` separator, unknown-option and multi-URL rejection, URL validation limited to http/https). Network path now uses `AbortSignal.timeout`, a routing user-agent/accept header, distinct timeout/request/read error messages, body-read failure handling, and echoes the normalized URL. Output now prints `retry-after`, case-insensitive JSON detection including JSON bodies without the header, surfaces worker `fix` plus the new-project checklist, strips style blocks/entities with an empty-body fallback, guards redirect-target URL parsing, expands plain-text hints from 3 to 15 worker-matched failure causes, and adds a 429 wait hint. `node --check` passes locally; ESLint on the file passes (exit 0). CLI probes verified: `--help` exit 0; invalid/non-http/multi-URL/unknown-option probes exit 2 with usage. |
