...
| Date | Check | Result |
|---|---|---|
| 2026-09-19 | Pre-existing rendered-html SSR failures fixed | LOCAL PASS - full `npm test` now **45 pass / 0 fail / 6 skip** (was 43/2/6). Root cause: `scripts/build-test.mjs`'s credential-free `envDir` (added in `ab98795`) only masks `.env` files — Vite still inherits shell-exported `VITE_*` variables, so the test bundle's auth configuration depended on the machine's environment: shells with `VITE_SUPABASE_*` set produced the "Checking your workspace session" gate the suite asserts, without them the demo shell rendered and 2 `tests/rendered-html.test.mjs` assertions failed. Fix in `vite.config.ts`: test builds now strip all shell `VITE_*` vars and pin non-secret placeholders (`https://test-suite.supabase.invalid` / `test-suite-anon-key`) via `define`, making the SSR gate deterministic on every machine; stale "demo experience" comments in `vite.config.ts` and `scripts/build-test.mjs` corrected. Verified: targeted rendered-html suite 10/10 pass; full suite 45/0/6; no shell exports required. |
| 2026-09-18 | TOID/unique_id survey URL split | LOCAL PASS / PUSHED - commit `7c3265b` is on `github/main`. `buildSurveyUrl` now resolves vendor placeholders into two separate IDs: `{{transaction_id}}`/`{{session_id}}`/`{[TOID]}`/`{[rid]}`/`[#scid#]` → per-entry ResearchOps session UUID (the TOID), `{{respondent_id}}`/`[unique_id]` → the supplier's permanent panelist reference; `rid` follows the client-return contract (session UUID) so every `?rid=` return maps to exactly one attempt. `redirects.ts` supplies both IDs at launch, `redirects.ts` line 175 default `rid` stays the session UUID, and the survey-setup editor preview reflects the split. New placeholder-split unit test passes; two-ID launch/return tests updated to assert `RID`=session UUID plus `UID`=supplier ref. Focused suites: 8/8 pass. Full `npm test` at the time: 43 pass / 2 fail / 6 skip (both later fixed, see 2026-09-19 row). `tsc --noEmit` and ESLint exit 0. |
| 2026-09-18 | Overview aligned with Project Center layout | LOCAL PASS / PUSHED - commit `2cbe612` is on `github/main`. Rebuilt `app/components/Dashboard.tsx` on the Project Center pattern: `project-center-page` wrapper, `reference-filter` grid with date-from/to, sort direction, scope/updated controls plus icon-button refresh/analytics/ledger/alerts actions, and the extra date-subtitle row. Replaced the custom `metric-strip`/`Metric` cards with a `Fieldwork overview` totals panel using the shared `outcome-grid project` cards. Kept respondent funnel/outcome tracking, supplier delivery, and live activity sections; analytics now requests `?from=&to=`, supplier order follows the sort control, and dead `Metric` code was removed. `tsc --noEmit` and ESLint on the file pass (exit 0). Full build/test and hosted uptake remain unverified. |
| 2026-09-18 | `scripts/check-route.mjs` hardening | LOCAL PASS / PUSHED - commit `f8c0264` is on `github/main`. Replaced positional `argv[2]` parsing with full argument parsing (`--diagnose`/`--json` order-independent, `--timeout=<ms>` with 120s cap, `--help`/`-h`, `--` separator, unknown-option and multi-URL rejection, URL validation limited to http/https). Network path uses `AbortSignal.timeout`, routing user-agent/accept headers, distinct timeout/request/read errors, and echoes the normalized URL. Output prints `retry-after`, detects JSON case-insensitively, surfaces worker `fix` plus checklist, strips style/entities, guards redirect parsing, expands plain-text hints from 3 to 15, and adds a 429 hint. `node --check` and ESLint pass; CLI probes verified (`--help` exit 0; invalid/non-http/multi-URL/unknown-option exit 2). |
...
### 2026-09-18 - TOID/unique_id survey URL split (PureProfile/Sago style)

- Explained the screenshot's two-ID system (`[TOID]` per-entry transaction vs `[unique_id]` permanent panelist) and implemented the same split: `worker/domain/survey-url.ts` resolves `{{transaction_id}}`/`{{session_id}}`, `{[TOID]}`, `{[rid]}`, `[#scid#]` to the per-entry ResearchOps session UUID and `{{respondent_id}}`/`[unique_id]` to the supplier's original reference; bare `{{...}}` keys still fall through to eligibility answers (for example `{{country}}`).
- `worker/routes/redirects.ts` now passes `transaction_id: sessionId, respondent_id: respondentRef` into the launch context (previously `respondent_id` was incorrectly the session UUID). The launch-time default `rid` fallback stays the session UUID, keeping the client-return contract intact. `worker/routes/survey-setup.ts`, `ProjectSurveySetupEditor` preview, and the create-project hint text were aligned to the split; tests were updated to assert `RID`=session UUID plus `UID`=supplier ref and a dedicated placeholder-split unit test covers `{[TOID]}`, `[unique_id]`, `{[rid]}`, `[#scid#]`, and `{{project_code}}`.
- Verification: focused suites 8/8 pass; full `npm test` 43 pass / 2 fail / 6 skip with both failures being pre-existing `tests/rendered-html.test.mjs` SSR session-check assertions — re-verified failing on a clean HEAD build via stash → build-test → test → stash pop, so they are unrelated to this change and remain an open defect to fix separately. `tsc --noEmit` and ESLint exit 0. Committed as `7c3265b` and pushed to `origin/main`.
- Current task: triage and fix the pre-existing rendered-html SSR failures. Next: continue the existing Current focus (`039` live checks, `038` proofs, hosted uptake verification).

### 2026-09-19 - Deterministic test SSR build (rendered-html failures fixed)

- Triaged the 2 pre-existing `rendered-html.test.mjs` failures: the "Checking your workspace session" gate needs the test bundle to be auth-configured, but `scripts/build-test.mjs`'s credential-free `envDir` (from `ab98795`) only masks `.env` files while Vite still inherits shell-exported `VITE_*` variables — so the suite silently depended on the developer's machine environment.
- Fixed in `vite.config.ts`: `RESEARCHOPS_TEST_BUILD` builds now strip all shell `VITE_*` variables and pin non-secret placeholder Supabase values (`https://test-suite.supabase.invalid` / `test-suite-anon-key`) through `define`, so the SSR gate renders deterministically everywhere and the bundle can never reach real infrastructure. Stale "demo experience" comments in `vite.config.ts` and `scripts/build-test.mjs` were corrected.
- Verification: targeted `tests/rendered-html.test.mjs` 10/10 pass; full `npm test` **45 pass / 0 fail / 6 skip** (Docker/Supabase-gated skips only) — first fully green suite since the regression landed in `ab98795`. Tracker rows and this log updated; commit and push are the next step.
- Current task: commit and push this fix, then continue the existing Current focus (`039` live checks, `038` proofs, hosted uptake verification).

### 2026-09-18 - Overview aligned with Project Center

- Rebuilt the Overview (`/dashboard`, `app/components/Dashboard.tsx`) on the Project Center pattern without touching its routing or data contract: `project-center-page` wrapper, `reference-filter` grid with date-from/to plus summary-order/scope/updated controls, icon-button refresh/analytics/ledger/alerts actions, and the date-subtitle extra row.
- Replaced bespoke `page-head` + `metric-strip`/`Metric` cards with a `Fieldwork overview` totals panel using the shared `outcome-grid project` cards. Kept respondent funnel/outcome tracking, supplier delivery, and live-activity sections; analytics now requests `?from=&to=`, supplier order follows the sort control, and dead metric code was removed.
- Verification: `tsc --noEmit` exit 0; ESLint on the file exit 0. Full build/test and hosted uptake remain unverified. Committed as `2cbe612` and pushed to `origin/main`.
- Current task: confirm hosted uptake of `2cbe612`, then continue the existing Current focus (`039` live checks, `038` proofs, fieldwork capture/workbook verification).

### 2026-09-18 - `check-route.mjs` hardening

- Hardened `scripts/check-route.mjs` without changing its routing contract: order-independent `--diagnose`/`--json`, `--timeout=<ms>` (20s default, 120s cap), `--help`/`-h`, `--` separator, unknown-option/multi-URL rejection with exit 2, and http(s)-only URL validation.
- Made the network path defensive: `AbortSignal.timeout`, routing user-agent/accept headers, separate timeout/request/body-read errors, normalized URL echo, `retry-after` output, case-insensitive JSON detection (including header-less JSON), worker `fix` + checklist surfacing, style/entity-aware HTML stripping, guarded redirect parsing, 15 worker-matched plain-text hints, and a 429 hint.
- Verification: ESLint on the file passes (exit 0); `node --check` is clean; `--help` exits 0; invalid/non-http/multi-URL/unknown-option probes exit 2 with usage. No live routing hit was run in this session; full `npm test` remains unrun.
- Current task: continue the existing Current focus (`039` live checks, `038` proofs, fieldwork capture/workbook verification). Next: commit and push this bounded script fix.
