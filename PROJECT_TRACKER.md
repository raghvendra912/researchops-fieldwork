...
2. Prove the remaining release checks now that hosted two-ID routing is verified end-to-end (`ROP-1141` production run): authenticated hosted `/api/projects` list read and Project Center load, `038` ownership constraints, audit records, role permissions, and tenant isolation; then probe the `supplier_scoped_ref_ready` capability RPC and run its live equal-reference, concurrent-quota, terminal-outcome, replay, and assignment-history tests.
