...
| Date | Check | Result |
|---|---|---|
| 2026-09-18 | TOID/unique_id survey URL split | LOCAL PASS / PUSHED - commit pending at write time. `buildSurveyUrl` now resolves vendor placeholders into two separate IDs: `{{transaction_id}}`/`{{session_id}}`/`{[TOID]}`/`{[rid]}`/`[#scid#]` → per-entry ResearchOps session UUID (the TOID), `{{respondent_id}}`/`[unique_id]` → the supplier's permanent panelist reference; `rid` follows the client-return contract (session UUID) so every `?rid=` return maps to exactly one attempt. `redirects.ts` supplies both IDs at launch, `redirects.ts` line 175 default `rid` stays the session UUID, and the survey-setup editor preview reflects the split. New placeholder-split unit test passes; two-ID launch/return tests updated to assert `RID`=session UUID plus `UID`=supplier ref. Focused suites: 8/8 pass. Full `npm test`: 43 pass / 2 fail / 6 skip, where both failures are the pre-existing `tests/rendered-html.test.mjs` SSR session-check assertions re-verified failing on a clean HEAD build (stash → build → test → pop), so they are unrelated to this change. `tsc --noEmit` and ESLint exit 0. |
...
- Verification: focused suites 8/8 pass; full `npm test` 43 pass / 2 fail / 6 skip with both failures being pre-existing `tests/rendered-html.test.mjs` SSR session-check assertions — re-verified failing on a clean HEAD build via stash → build-test → test → stash pop, so they are unrelated to this change and remain an open defect to fix separately. `tsc --noEmit` and ESLint exit 0.
- Current task: push this split, then continue the existing Current focus (`039` live checks, `038` proofs, hosted uptake verification). Next: triage and fix the pre-existing rendered-html SSR failures.
