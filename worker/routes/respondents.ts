...
export async function handleRespondentsApi(request: Request, pathname: string, env: SupabaseEnv): Promise<Response | null> {
  ...
  try {
    ...
    const data = rows.map((row) => {
      ...
      return { id: row.id, respondentRef: row.respondent_ref, status: row.status, isTest: row.is_test === true, startedAt: row.started_at, completedAt: row.completed_at, durationSeconds: Number.isFinite(completed) ? Math.max(0, Math.round((completed - started) / 1000)) : null, projectCode: project?.project_code, projectName: project?.project_name, supplierName: supplier?.name, supplierCpi: Number(assignment?.supplier_cpi ?? 0), events: events.map((event) => ({ id: event.id, eventType: event.event_type, occurredAt: event.occurred_at, providerTransactionId: event.provider_transaction_id, metadata: event.metadata })) };
    });
