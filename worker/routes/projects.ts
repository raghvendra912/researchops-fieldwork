...
import { supplierShortTemplate } from "../domain/routing-links";
...
function assignmentRow(row: { id?: string; supplier_id: string; supplier_project_id: string | null; supplier_cpi: number | string; target_quota: number; status: SupplierAssignment["status"]; suppliers?: { name: string; redirect_token?: string; redirect_mode?: string } | { name: string; redirect_token?: string; redirect_mode?: string }[] | null; supplier_name?: string; test_starts?: number; test_completes?: number; test_terminates?: number; test_over_quota?: number; test_quality_term?: number; starts?: number; reached?: number; completes?: number; terminates?: number; over_quota?: number; quality_term?: number; incidence_rate?: number | string | null; conversion_rate?: number | string | null; cost?: number | string }, origin?: string, projectCode?: string) {
  ...
  // Tool-only routing: every new project gets its supplier links from this
  // helper, never from hand-written URLs scattered across the UI.
...
export async function handleProjectsApi(request: Request, pathname: string, env: ProjectApiEnv): Promise<Response | null> {
  ...
  try {
    if (pathname === "/api/projects/specifications" && request.method === "GET") {
      ...
      const canExportLinks = workspacePermissions.operate.includes(access.membership.role as "OWNER" | "ADMIN" | "PM"); const supplierFields = canExportLinks ? "name,redirect_token,redirect_mode" : "name,redirect_mode";
      type SpecRow = { id: string; project_code: string; survey_url: string | null; test_survey_url: string | null; survey_parameters: unknown; project_markets: Array<{ country_code: string; language_code: string; target_quota: number; expected_loi_minutes: number; expected_ir: number | string }>; project_suppliers: Array<{ supplier_project_id: string | null; supplier_cpi: number | string; target_quota: number; status: string; suppliers: { name: string; redirect_token?: string; redirect_mode: string } | { name: string; redirect_token?: string; redirect_mode: string }[] }>; project_eligibility_rules: Array<{ variable_key: string; operator: string; values: string[]; required: boolean; active: boolean; sort_order: number }>; project_quota_cells: Array<{ id: string; name: string; target_quota: number; priority: number; active: boolean; conditions: unknown }> };
      const rows = await supabaseJson<SpecRow[]>(env, `/rest/v1/projects?select=id,project_code,survey_url,test_survey_url,survey_parameters,project_markets(country_code,language_code,target_quota,expected_loi_minutes,expected_ir),project_suppliers(supplier_project_id,supplier_cpi,target_quota,status,suppliers(${supplierFields})),project_eligibility_rules(variable_key,operator,values,required,active,sort_order),project_quota_cells(id,name,target_quota,priority,active,conditions)&project_code=in.(${codes.join(",")})`, access.authorization);
      const projectIds = rows.map((row) => row.id); const quotaMetrics = projectIds.length ? await supabaseJson<Array<{ quota_cell_id: string; completes: number; reserved: number; remaining: number }>>(env, `/rest/v1/project_quota_cell_metrics?select=quota_cell_id,completes,reserved,remaining&project_id=in.(${projectIds.join(",")})`, access.authorization) : []; const quotaByCell = new Map(quotaMetrics.map((item) => [item.quota_cell_id, item])); const origin = new URL(request.url).origin;
      const data = rows.map((row) => ({ projectCode: row.project_code, liveSurveyUrl: row.survey_url ?? "", testSurveyUrl: row.test_survey_url ?? "", surveyParameters: Array.isArray(row.survey_parameters) ? row.survey_parameters : [], markets: row.project_markets.map((market) => ({ countryCode: market.country_code, languageCode: market.language_code, targetQuota: market.target_quota, expectedLoiMinutes: market.expected_loi_minutes, expectedIr: Number(market.expected_ir) })), suppliers: row.project_suppliers.map((assignment) => { const supplier = Array.isArray(assignment.suppliers) ? assignment.suppliers[0] : assignment.suppliers; const liveLink = supplier?.redirect_token ? `${origin}/r/supplier/${supplier.redirect_token}/live?project=${row.project_code}&respondent={{respondent_id}}` : ""; return { supplierName: supplier?.name ?? "Supplier", supplierProjectId: assignment.supplier_project_id ?? "", supplierCpi: Number(assignment.supplier_cpi), targetQuota: assignment.target_quota, status: assignment.status, redirectMode: supplier?.redirect_mode ?? "", testLink: liveLink ? `${liveLink}&mode=test` : "", liveLink, shortLink: supplier?.redirect_token ? supplierShortTemplate(origin, supplier.redirect_token, row.project_code) : "" }; }), eligibilityRules: row.project_eligibility_rules.map((rule) => ({ variableKey: rule.variable_key, operator: rule.operator, values: rule.values, required: rule.required, active: rule.active, sortOrder: rule.sort_order })), quotaCells: row.project_quota_cells.map((cell) => ({ name: cell.name, targetQuota: cell.target_quota, priority: cell.priority, active: cell.active, conditions: cell.conditions, ...quotaByCell.get(cell.id) })) }));
      return Response.json({ data, meta: { source: "supabase" } }, { headers: { "cache-control": "private, no-store" } });
    }
    ...
    if (assignmentsMatch && request.method === "GET") {
      ...
      const capability = await getProjectCapabilities(env, access.authorization, projectCode); const supplierFields = capability?.can_operate ? "name,redirect_token,redirect_mode" : "name,redirect_mode";
      const rows = await supabaseJson<Array<{ id: string; supplier_id: string; supplier_project_id: string | null; supplier_cpi: number | string; target_quota: number; status: SupplierAssignment["status"]; suppliers: { name: string; redirect_token?: string; redirect_mode?: string } | { name: string; redirect_token?: string; redirect_mode?: string }[]; projects: { project_code: string } }>>(env, `/rest/v1/project_suppliers?select=id,supplier_id,supplier_project_id,supplier_cpi,target_quota,status,suppliers(${supplierFields}),projects!inner(project_code)&projects.project_code=eq.${encodeURIComponent(projectCode)}&order=created_at.asc`, access.authorization);
      ...
      return Response.json({ data: rows.map((row) => assignmentRow({ ...row, ...metrics.get(row.supplier_id) }, capability?.can_operate ? new URL(request.url).origin : undefined, projectCode)), meta: { source: "supabase", canOperate: capability?.can_operate === true } });
    }
    ...
    const message = databaseError?.status === 404
      ...
      : databaseError?.detail
          ? `The database rejected ${request.method === "GET" ? "project data" : "the project"}: ${databaseError.detail}${reference}`
