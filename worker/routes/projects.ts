...
export async function handleProjectsApi(request: Request, pathname: string, env: ProjectApiEnv): Promise<Response | null> {
  ...
  try {
    ...
    if (assignmentsMatch && request.method === "GET") {
      ...
      const metricRows = await supabaseJson<Array<{ supplier_id: string; starts: number; reached: number; completes: number; terminates: number; over_quota: number; quality_term: number; incidence_rate: number | string | null; cost: number | string }>>(env, `/rest/v1/project_supplier_event_metrics?select=supplier_id,starts,reached,completes,terminates,over_quota,quality_term,incidence_rate,cost&project_code=eq.${encodeURIComponent(projectCode)}`, access.authorization);
