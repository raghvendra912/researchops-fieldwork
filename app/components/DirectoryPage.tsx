...
type DirectoryRecord = {
  ...
  redirects: Redirects; links?: { complete?: string; terminate?: string; quotaFull?: string; securityTerminate?: string; test?: string; live?: string };
};
...
export function DirectoryPage({ kind, eyebrow, title, subtitle, action }: { kind: DirectoryKind; eyebrow: string; title: string; subtitle: string; action: string }) {
  ...
  const [query, setQuery] = useState(""); const [formOpen, setFormOpen] = useState(false); const [editing, setEditing] = useState<DirectoryRecord | null>(null); const [error, setError] = useState(""); const [source, setSource] = useState<"mock" | "supabase">("mock"); const [canOperate, setCanOperate] = useState(!configured); const [copied, setCopied] = useState("");
  ...
  const filtered = useMemo(() => { const term = query.trim().toLowerCase(); return records.filter((record) => !term || [record.name, record.code, record.contactName, record.contactEmail].some((value) => value.toLowerCase().includes(term))); }, [query, records]);
  ...
  async function save(event: FormEvent<HTMLFormElement>) {
    ...
    const payload = {
      ...
      ...(kind === "suppliers" ? { redirectMode: form.get("redirectMode") } : {}),
      redirects: { completeUrl: form.get("completeUrl"), terminateUrl: form.get("terminateUrl"), quotaFullUrl: form.get("quotaFullUrl"), securityTerminateUrl: form.get("securityTerminateUrl") },
    };
    try {
  ...
  return <>
    ...
    {formOpen ? <section className="panel directory-form">
      <div className="section-head"><div><h2>{editing ? `Edit ${editing.name}` : action}</h2><p>Contact details and redirect destinations are stored with the workspace record.</p></div><button className="button small ghost" type="button" onClick={() => setFormOpen(false)}>Close</button></div>
      <form className="form-grid three" onSubmit={save}>
        ...
        {kind === "suppliers" ? <div className="field"><label htmlFor="redirect-mode">Redirect mode</label><select className="control" id="redirect-mode" name="redirectMode" defaultValue={editing?.redirectMode ?? "STATIC"}><option value="STATIC">Static — reused for every project</option><option value="DYNAMIC">Dynamic — supports project/respondent placeholders</option></select></div> : null}
        <div className="field full"><h3>Redirect destinations</h3><p className="panel-note">Dynamic URLs may use {"{{respondent_id}}"} and {"{{project_id}}"}.</p></div>
        <Field label="Complete URL" name="completeUrl" value={editRedirects.completeUrl} type="url" /><Field label="Terminate URL" name="terminateUrl" value={editRedirects.terminateUrl} type="url" /><Field label="Quota-full URL" name="quotaFullUrl" value={editRedirects.quotaFullUrl} type="url" /><Field label="Security-terminate URL" name="securityTerminateUrl" value={editRedirects.securityTerminateUrl} type="url" />
        {editing?.links ? <div className="field full"><h3>Masked ResearchOps links</h3><div className="info-list">{Object.entries(editing.links).map(([label, link]) => link ? <div className="info-row" key={label}><span>{humanize(label)}</span><code>{link}</code><button className="button small ghost" type="button" onClick={() => void copyLink(label, link)}>{copied === label ? "Copied" : "Copy"}</button></div> : null)}</div></div> : null}
    ...
    <section className="panel"><div className="panel-head"><h2 className="panel-title">{title} directory</h2><div className="field search-wrap" style={{ width: 260 }}><input aria-label={`Search ${title.toLowerCase()}`} className="control search-control" placeholder={`Search ${title.toLowerCase()}`} value={query} onChange={(event) => setQuery(event.target.value)} /></div></div>
      <div className="table-wrap"><table className="data-table" style={{ minWidth: 860 }}><thead><tr><th>Name</th><th>Contact</th>{kind === "suppliers" ? <th>Redirect mode</th> : null}<th>Projects</th><th>Status</th><th>Links</th>{canOperate ? <th>Actions</th> : null}</tr></thead><tbody>{filtered.map((record) => <tr key={record.id}><td className="project-name-cell"><strong>{record.name}</strong><span>{record.code}</span></td><td className="project-name-cell"><strong>{record.contactName || "Not set"}</strong><span>{record.contactEmail || record.phone || "No contact details"}</span></td>{kind === "suppliers" ? <td>{record.redirectMode}</td> : null}<td className="rate">{record.projectCount}</td><td><span className={`status-pill status-${record.status}`}>{record.status}</span></td><td>{record.links ? <button className="button small ghost" type="button" onClick={() => openEdit(record)}>View links</button> : "Restricted"}</td>{canOperate ? <td><div className="row-actions"><button className="button small ghost" type="button" onClick={() => openEdit(record)}>Edit</button><button className="button small ghost" type="button" onClick={() => void toggleStatus(record)}>{record.status === "ACTIVE" ? "Deactivate" : "Activate"}</button></div></td> : null}</tr>)}{filtered.length === 0 ? <tr><td colSpan={canOperate ? (kind === "suppliers" ? 7 : 6) : (kind === "suppliers" ? 6 : 5)} style={{ textAlign: "center", padding: 32, color: "var(--muted)" }}>No matching records.</td></tr> : null}</tbody></table></div>
