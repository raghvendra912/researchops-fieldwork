...
export function DirectoryPage({ kind, eyebrow, title, subtitle, action }: { kind: DirectoryKind; eyebrow: string; title: string; subtitle: string; action: string }) {
  ...
  return <>
    ...
    {linkRecord ? <div className="modal-backdrop"><section aria-modal="true" className="panel links-modal" role="dialog" aria-label={`${linkRecord.name} links`}><div className="section-head"><div><div className="eyebrow">Client handoff</div><h2>{linkRecord.name} redirect links</h2><p>These same-domain endpoints are auto-generated. Final destinations remain hidden behind ResearchOps routing.</p></div><button className="button small ghost" type="button" onClick={() => setLinkRecord(null)}>Close</button></div>
      ...
      {kind === "clients" ? <section className="variable-box"><div className="section-head"><div><h3>Redirect variables</h3><p>Define each reusable placeholder once. Links use the format {"{{variable_name}}"}.</p></div>{canOperate ? <button className="button small ghost" type="button" onClick={() => setVariables((current) => [...current, { name: "client_id", source: "SYSTEM", defaultValue: "", required: false }])} disabled={variables.length >= 8}>Add variable</button> : null}</div>
