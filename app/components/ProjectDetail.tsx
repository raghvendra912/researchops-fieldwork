...
export function ProjectDetail({ projectId }: { projectId: string }) {
  ...
  return <>
    ...
    <div className="split-grid"><ProjectSuppliersEditor projectId={project.id} configured={configured} token={token} canOperate={canOperate} /><section className="panel"><div className="panel-head"><h2 className="panel-title">Project details</h2>{canOperate ? <button className="button small ghost" type="button" onClick={() => setEditing((value) => !value)}>{editing ? "Cancel" : "Edit"}</button> : null}</div>
    ...
    </section></div><ProjectMarketsEditor projectId={project.id} configured={configured} token={token} canOperate={canOperate} />
  </>;
}
