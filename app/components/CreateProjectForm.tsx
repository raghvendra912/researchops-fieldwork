...
export function CreateProjectForm() {
  ...
  return <>
    ...
    {createdId ? <div className="success-banner"><span>Project draft {createdId} created and assigned to the signed-in operator.</span><Link href={`/projects/${createdId}`}>Open project →</Link></div> : null}
    ...
    <form className="form-shell" onSubmit={submit}>
      ...
      <div className="panel form-panel">
        <section className="form-section"><div className="section-head"><div><h2>Project information</h2><p>Choose an existing client and controlled study classification.</p></div><span className="status-pill status-PENDING">DRAFT</span></div><div className="form-grid">
        ...
        <section className="form-section"><div className="section-head"><div><h2>Market & quota</h2><p>Define the first audience. More markets can be added after creation.</p></div></div><div className="form-grid three">
          <div className="field"><label htmlFor="country">Country</label><select className="control" id="country" name="country" defaultValue="IN"><option value="IN">India</option><option value="US">United States</option><option value="GB">United Kingdom</option><option value="DE">Germany</option><option value="AU">Australia</option></select></div>
          <div className="field"><label htmlFor="language">Language</label><select className="control" id="language" name="language" defaultValue="en"><option value="en">English</option><option value="hi">Hindi</option><option value="de">German</option></select></div>
        ...
        <div className="form-actions"><span className="save-note">A dated draft is created when you submit.</span><div className="head-actions"><Link className="button ghost" href="/projects">Cancel</Link><button className="button primary" type="submit" disabled={submitting || !canOperate}>{submitting ? "Creating…" : "Create project →"}</button></div></div>
      </div>
    </form>
  </>;
}
